...
Функция ПолучитьПутьКВерсииПлатформы(Знач ВерсияПлатформы, Знач Разрядность = Неопределено) Экспорт
	...
	Если Лев(ВерсияПлатформы, 1) <> "8" Тогда
...
//	 Объекты - Структура - см. ОбъектыОтчетаСравненияМетаданных
...
Процедура ПолучитьОтчетОСравненииКонфигурацииСФайлом(
	...
	Если Объекты = Неопределено Тогда
		Объекты = ОбъектыОтчетаСравненияМетаданных();
	КонецЕсли;
	Если ФлагСтруктуры(Объекты, "ИзмененныеОбъектыМетаданных") Тогда
		ПараметрыЗапуска.Добавить("-IncludeChangedObjects");
	КонецЕсли;
	Если ФлагСтруктуры(Объекты, "УдаленныеОбъектыМетаданных") Тогда
		ПараметрыЗапуска.Добавить("-IncludeDeletedObjects");
	КонецЕсли;
	Если  ФлагСтруктуры(Объекты, "ДобавленныеОбъектыМетаданных") Тогда
		ПараметрыЗапуска.Добавить("-IncludeAddedObjects");
	КонецЕсли;
...
Процедура Инициализация()
	...
	ПутьКПлатформе1С(ПолучитьПутьКВерсииПлатформы("8", РазрядностьПлатформы.x64x86));
...
// Возвращает объекты для параметров сравнения метаданных
// Параметры:
//	ИзмененныеОбъекты - Булево - включать в отчет измененные подчиненные объекты
//	УдаленныеОбъекты - Булево - включать в отчет измененные подчиненные объекты
//	ДобавленныеОбъекты - Булево - включать в отчет измененные подчиненные объекты	
// Возвращаемое значение:
//  - Структура
//		* ИзмененныеОбъектыМетаданных	- Булево
//		* УдаленныеОбъектыМетаданных	- Булево
//		* ДобавленныеОбъектыМетаданных	- Булево
Функция ОбъектыОтчетаСравненияМетаданных(ИзмененныеОбъекты = Истина,
	УдаленныеОбъекты = Истина, ДобавленныеОбъекты = Истина) Экспорт

	ОбъектыОтчета = Новый Структура;
	ОбъектыОтчета.Вставить("ИзмененныеОбъектыМетаданных", ИзмененныеОбъекты);
	ОбъектыОтчета.Вставить("УдаленныеОбъектыМетаданных", УдаленныеОбъекты);
	ОбъектыОтчета.Вставить("ДобавленныеОбъектыМетаданных", ДобавленныеОбъекты);

	Возврат ОбъектыОтчета;

КонецФункции

Функция ФлагСтруктуры(Структура, Ключ)

	ЗначениеКлюча = Неопределено;
	Возврат Структура.Свойство(Ключ, ЗначениеКлюча) И ЗначениеКлюча = Истина;

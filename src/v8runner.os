...
Процедура ПолучитьОтчетОСравненииКонфигурацииСФайлом(
	...
	Знач ИмяРасширения = "",
	Знач ПутьКФайлуПервойКонфигурации = Неопределено,
	...
	КонецЕсли;
	Если Объекты.Свойство("ИзмененныеОбъектыМетаданных") Тогда
		ПараметрыЗапуска.Добавить("-IncludeChangedObjects");
	КонецЕсли;
	Если Объекты.Свойство("УдаленныеОбъектыМетаданных") Тогда
		ПараметрыЗапуска.Добавить("-IncludeDeletedObjects");
	КонецЕсли;
	Если Объекты.Свойство("ДобавленныеОбъектыМетаданных") Тогда
	...
	Если НЕ ПараметрыХранилища = Неопределено Тогда
		ПараметрыЗапуска.Добавить("/ConfigurationRepositoryF """ + ПараметрыХранилища.СтрокаПодключенияХранилище + """");
		ПараметрыЗапуска.Добавить("/ConfigurationRepositoryN """ + ПараметрыХранилища.ПользовательХранилища + """");

		Если ЗначениеЗаполнено(ПараметрыХранилища.ПарольХранилища) Тогда
			ПараметрыЗапуска.Добавить("/ConfigurationRepositoryP """ + ПараметрыХранилища.ПарольХранилища + """");
		КонецЕсли;
	КонецЕсли;
...
// Возвращаемое значение:
//  - Структура
//		* ИзмененныеОбъектыМетаданных	- Булево - Измененные объекты метаданных
//		* УдаленныеОбъектыМетаданных	- Булево - Имя пользователя хранилища
//		* ДобавленныеОбъектыМетаданных	- Булево - Пароль пользователя хранилища
Функция ОбъектыОтчетаСравненияМетаданных()

	ОбъектыОтчета = Новый Структура;
	ОбъектыОтчета.Вставить("ИзмененныеОбъектыМетаданных", Истина);
	ОбъектыОтчета.Вставить("УдаленныеОбъектыМетаданных", Истина);
	ОбъектыОтчета.Вставить("ДобавленныеОбъектыМетаданных", Истина);

...
Функция ПолучитьСписокТестов(Тестирование) Экспорт
	...
	Попытка
		ПутьПриложения = УправлениеКонфигуратором.ПолучитьПутьКВерсииПлатформы("8.3.23.1782");
		Если Не ЗначениеЗаполнено(ПутьПриложения) Тогда
			ВызватьИсключение "Не найден путь к платформе 8.3.23.1782";
		КонецЕсли;
		ПутьПриложения = УправлениеКонфигуратором.ПолучитьПутьКВерсииПлатформы("8.3.23.1437");
		Если Не ЗначениеЗаполнено(ПутьПриложения) Тогда
			ВызватьИсключение "Не найден путь к платформе 8.3.23.1437";
		КонецЕсли;
...
Процедура ТестДолжен_ПроверитьУстановкуПутиКПлатформе() Экспорт
	...
	Утверждения.ПроверитьЛожь(ПустаяСтрока(ПутьДоТестовойВерсии), "Не найдена платформа 8.3.23.1782");
	...
	Утверждения.ПроверитьЛожь(ПустаяСтрока(ПутьДоТестовойВерсии), "Не найдена платформа 8.3.23.1437");
	Утверждения.ПроверитьРавенство(ПутьДоТестовойВерсии, УправлениеКонфигуратором.ПутьКПлатформе1С(ПутьДоТестовойВерсии));
